...
void main() {
  test(
    'ClearSearchHistoryUsecase clears rows by target [assertion 1/3]',
    () async {
  ...
  test(
    'ClearSearchHistoryUsecase clears rows by target [assertion 2/3]',
    () async {
      final db = AppDatabase(NativeDatabase.memory());
      addTearDown(db.close);
      final repository = SearchHistoryRepository(db.searchHistoriesDao);
  ...
  test(
    'ClearSearchHistoryUsecase clears rows by target [assertion 3/3]',
    () async {
      final db = AppDatabase(NativeDatabase.memory());
      addTearDown(db.close);
      final repository = SearchHistoryRepository(db.searchHistoriesDao);
