...
| Tool list | Tablet tool-list uses vertical items, active container, icon + label + formula, with no ellipsis when enough width exists | `_CalcToolList` renders BMI/eGFR/CrCl with built-in Flutter `Icons.*`; formula is below the label and wraps instead of using `TextOverflow.ellipsis`; mono text has NotoSansJP fallback for superscripts | iPad portrait/landscape goldens show full formulas and visible icons; widget test asserts the eGFR formula is not ellipsized; design contract asserts production Calc UI has no Material Symbols dependency | pass |
